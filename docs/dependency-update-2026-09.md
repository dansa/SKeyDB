...
- **Node types:** addressed in a separate dependency commit. Direct `@types/node` now uses `^24.13.3`, matching the Node 24 runtime major in `.node-version` and CI. TypeScript compilation and the production build pass; no source compatibility edits were necessary. The lockfile also resolves the corresponding undici-types dependency. Audit remains at zero vulnerabilities.
