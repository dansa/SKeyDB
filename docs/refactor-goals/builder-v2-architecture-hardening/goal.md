...
| C3: V2 selection/editing-mode boundary | TypeScript scouts | queued | `BuilderV2ModelTypes.ts`, `useBuilderV2Model.ts`, selection helper module, focused model/page tests | `activeSelection`, `activeTeamTarget`, and `pickerTab` are independent fields for one editing mode and can drift into impossible states. Selection can also reference non-existent slots. | Recommended next by J3/scout after W2; keep separate from shared `WheelSlotIndex` and layout/DnD scope. |
...
| C6: Pure picker option query modules | React/complexity scouts | queued | `useBuilderV2Model.ts`, new picker query module(s), `BuilderV2ModelTypes.ts`, focused model tests | Four picker option pipelines are large `useMemo` blocks coupled to hook state and hard to test directly. | Queue after command boundary unless picker work becomes the next pain point. |
...
Current tranche decision: W1 implemented C1 Assignment behavior characterization, then W2 implemented C2 Builder V2 loadout command resolver extraction. J3 is active for the next post-command-boundary slice; current scout recommendation is C3 first, then C6.
