...
describe('selection-state', () => {
  ...
  it('toggles wheel selection for the same slot/index', () => {
    expect(
      toggleWheelSelection({kind: 'wheel', slotId: 'slot-1', wheelIndex: 0}, 'slot-1', 0),
    ).toBeNull()
  ...
  it('enables wheel-active landing only when wheel selection is active', () => {
    expect(
      shouldSetActiveWheelOnPickerAssign({kind: 'wheel', slotId: 'slot-1', wheelIndex: 0}),
    ).toBe(true)
  ...
  it('clears selection only when removed wheel was active', () => {
    expect(
      nextSelectionAfterWheelRemoved({kind: 'wheel', slotId: 'slot-1', wheelIndex: 0}, 'slot-1', 0),
    ).toBeNull()
    expect(
      nextSelectionAfterWheelRemoved({kind: 'wheel', slotId: 'slot-1', wheelIndex: 0}, 'slot-1', 1),
    ).toEqual({
      kind: 'wheel',
      slotId: 'slot-1',
      wheelIndex: 0,
