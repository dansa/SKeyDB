...
function collectTeamUsage(teams: Team[]): TeamUsageSnapshot {
  ...
  teams.forEach((team) => {
    ...
    team.slots.forEach((slot) => {
      if (slot.awakenerId) {
        usage.awakenerKeys.add(getAwakenerIdentityKeyById(slot.awakenerId))
      }
...
function collectImportedAwakenerKeys(team: Team): Set<string> {
  return new Set(
    team.slots
      .map((slot) => slot.awakenerId)
      .filter((id): id is string => Boolean(id))
      .map((id) => getAwakenerIdentityKeyById(id)),
  )
}
...
function applyMoveStrategy(currentTeams: Team[], importedTeam: Team): Team[] {
  ...
  return currentTeams.map((team) => {
    const nextSlots = team.slots.map((slot) => {
      const slotAwakenerKey = slot.awakenerId ? getAwakenerIdentityKeyById(slot.awakenerId) : null
...
function applySkipStrategy(currentTeams: Team[], importedTeam: Team): Team {
  ...
  const nextSlots = importedTeam.slots.map((slot) => {
    if (slot.awakenerId && usage.awakenerKeys.has(getAwakenerIdentityKeyById(slot.awakenerId))) {
      return stripSlotAwakener(slot)
    }
