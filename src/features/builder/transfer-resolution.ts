...
function findAssignedAwakenerSlotId(
  ...
  for (const slot of nextSlots) {
    if (slot.awakenerId !== awakenerId) {
      continue
    }
    const previousSlot = previousById.get(slot.slotId)
    if (previousSlot?.awakenerId !== awakenerId) {
      return slot.slotId
    }
  }
  return nextSlots.find((slot) => slot.awakenerId === awakenerId)?.slotId ?? null
}
...
export function applyPendingTransfer(teams: Team[], pendingTransfer: PendingTransfer): Team[] {
  ...
  const moveResult = pendingTransfer.targetSlotId
    ? assignAwakenerToSlot(
        toTeam.slots,
        pendingTransfer.awakenerId,
        pendingTransfer.targetSlotId,
        awakenerById,
      )
    : assignAwakenerToFirstEmptySlot(toTeam.slots, pendingTransfer.awakenerId, awakenerById)
  if (moveResult.violation || moveResult.nextSlots === toTeam.slots) {
    return teams
  }

  const identityKey = getAwakenerIdentityKeyById(pendingTransfer.awakenerId)
...
export function applySupportTransfer(teams: Team[], pendingTransfer: PendingTransfer): Team[] {
  ...
  const supportSlotId =
    pendingTransfer.targetSlotId ??
    findAssignedAwakenerSlotId(toTeam.slots, assignResult.nextSlots, pendingTransfer.awakenerId)
  if (!supportSlotId) {
    return teams
  }
