...
export function BuilderDragOverlay({
  ...
  const activeDraggedAwakenerName = activeDraggedSlot
    ? getSlotAwakenerName(activeDraggedSlot)
    : undefined
  const activeDraggedAwakenerOwnedLevel = activeDraggedAwakenerName
    ? (ownedAwakenerLevelByName.get(activeDraggedAwakenerName) ?? null)
    : null
