...
function buildTeam(id: string, name: string, awakenerId?: string, posseId?: string): Team {
  return {
    id,
    name,
    posseId,
...
describe('usePendingResetTeamDialog', () => {
  it('resets empty teams immediately without pending dialog', () => {
    ...
    expect(result.current.teams[1]?.slots.every((slot) => !slot.awakenerId)).toBe(true)
  ...
  it('requires confirmation for non-empty active team and clears selection on confirm', () => {
    const initialTeams = [buildTeam('team-1', 'Team 1', 'awakener-0021', 'taverns-opening')]
    const {result} = renderHook(() => {
      const [teams, setTeams] = useState(initialTeams)
      const [selectionCleared, setSelectionCleared] = useState(false)
      const hook = usePendingResetTeamDialog({
    ...
    expect(result.current.teams[0]?.slots.every((slot) => !slot.awakenerId)).toBe(true)
