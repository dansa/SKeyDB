...
function getQuickLineupStepLabel(
  ...
  const awakenerName = slot ? getSlotAwakenerName(slot) : undefined
  const awakenerLabel = awakenerName
    ? formatAwakenerNameForUi(awakenerName)
...
function getSlotAwakenerLevel(slot: TeamSlot, awakenerLevelByName: Map<string, number>): number {
  ...
  return awakenerLevelByName.get(awakenerName) ?? 60
}
...
function getSlotAwakenerOwnedLevel(
  ...
  return ownedAwakenerLevelByName.get(awakenerName) ?? null
}
  ...
  return (
    <div className='px-4 pt-4 pb-1'>
      ...
      <div className='mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4'>
        {teamSlots.map((slot) => (
          <AwakenerCard
            key={`${slot.slotId}:${slot.awakenerId ?? 'empty'}`}
            activeKind={
