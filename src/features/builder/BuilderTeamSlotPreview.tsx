...
import {getSlotAwakenerName} from './slot-awakener'
  ...
  return (
    <>
      {slot.awakenerId ? (
        <>
          <img
            alt={`${getSlotAwakenerName(slot) ?? 'Awakener'} expanded team preview card`}
            className={`builder-team-slot-preview-card-art ${!isAwakenerOwned ? 'builder-picker-art-unowned' : ''}`}
            draggable={false}
            src={awakenerCardAsset ?? getAwakenerPortraitAsset(getSlotAwakenerName(slot) ?? '')}
          />
          <span
            className='pointer-events-none absolute inset-0 z-10 border'
            style={{borderColor: getRealmAccent(slot.realm)}}
...
export function BuilderTeamSlotPreview({
  ...
  const isAwakenerOwned =
    ...
    (ownedAwakenerLevelByName.get(getSlotAwakenerName(slot) ?? '') ?? null) !== null
  const covenantAsset = slot.covenantId ? getCovenantAssetById(slot.covenantId) : undefined
  const awakenerName = getSlotAwakenerName(slot)
  const awakenerCardAsset = awakenerName ? getAwakenerCardAsset(awakenerName) : undefined
  ...
  const {
    ...
    id: `${dropZoneId}:drag`,
    data: dragData,
    disabled: !enableDragAndDrop || !slot.awakenerId,
  })
