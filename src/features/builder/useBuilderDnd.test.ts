...
describe('useBuilderDnd', () => {
  ...
  it('routes team wheel drop to picker remove zone', () => {
    ...
    dnd.handleDragEnd({
      active: {
        data: {current: {kind: 'team-wheel', slotId: 'slot-1', wheelIndex: 0, wheelId: 'B01'}},
      },
  ...
  it('routes team wheel drop to another wheel target', () => {
    ...
    dnd.handleDragEnd({
      active: {
        data: {current: {kind: 'team-wheel', slotId: 'slot-1', wheelIndex: 1, wheelId: 'B01'}},
      },
      over: {id: wheelDropId},
    } as never)
  ...
  it('maps picker awakener wheel-zone drops back to slot id', () => {
    ...
    dnd.handleDragEnd({
      active: {data: {current: {kind: 'picker-awakener', awakenerId: 'awakener-0021'}}},
      over: {id: wheelDropId},
    } as never)

    expect(onDropPickerAwakener).toHaveBeenCalledWith('awakener-0021', 'slot-4')
  })
  ...
  it('ignores team-slot drops on non-slot ids (e.g. team rows)', () => {
    ...
    dnd.handleDragEnd({
      active: {data: {current: {kind: 'team-slot', slotId: 'slot-1', awakenerId: 'awakener-0021'}}},
      over: {id: 'team-2'},
    } as never)
