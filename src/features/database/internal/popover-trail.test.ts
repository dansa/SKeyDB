...
describe('popover trail stack helpers', () => {
  it('inserts a new nested entry directly after the source entry and truncates subsequent entries', () => {
    const next = insertTrailEntryAfterIndex(
      [skillEntry('s1'), skillEntry('s2'), skillEntry('s3')],
      1,
      skillEntry('retain'),
    )
    expect(next.map((e) => e.key)).toEqual(['s1', 's2', 'retain'])
  })
  it('treats reinserting an existing nested entry by truncating up to that entry', () => {
    const stack = [skillEntry('s1'), skillEntry('s2'), skillEntry('s3')]
    const next = insertTrailEntryAfterIndex(stack, 1, skillEntry('s2'))
    expect(next.map((e) => e.key)).toEqual(['s1', 's2'])
  })
  it('closes the selected entry and all subsequent entries in the trail', () => {
    const next = closeTrailFromIndex([skillEntry('s1'), skillEntry('s2'), skillEntry('s3')], 1)
    expect(next.map((e) => e.key)).toEqual(['s1'])
  })
  it('closes all entries when closing the root entry', () => {
